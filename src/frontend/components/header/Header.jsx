...
const Header = () => {
  ...
  const [cartItens, setCartItens, cartPrice, setCartPrice] = useContext(CartContext);
  const [showCart, setShowCart] = useState(false);
  const [ purchaseName, setPurchaseName, purchaseAddress, setPurchaseAddress, purchasePhone, setPurchasePhone, purchaseProducts, setPurchaseProducts, purchasePrice, setPurchasePrice, submitPurchase ] = useContext(PurchaseContext);
  ...
  const removeFromCart = (item) => {
    if (confirm("Tem certeza que quer remover esse item?")) {
        ...
        if (isNaN(price) || isNaN(number)) {
            console.log('Erro: Um ou mais valores são indefinidos ou não são números.');
  ...
  return (
    <div>
      <S.Container>
        <S.Logo src={logo} />
        <div>
          <S.CartBtn style={{display: user ? 'inline' : 'none', backgroundColor: !showCart ? 'transparent' : '#333' }} onClick={cartClick}>Carrinho</S.CartBtn>
          <S.PurchasesBtn style={{display: user ? 'inline' : 'none'}} onClick={purchasesClick}>Pedidos</S.PurchasesBtn>
          <S.LoginBtn onClick={user ? goToProfile : goToLogin}>{user ? "Perfil" : "Login"}</S.LoginBtn>
          <S.CartItens style={showCart ? {transform: 'translateX(0)'} : {transform: 'translateX(100%)'}}>
            <h1>Carrinho</h1>
